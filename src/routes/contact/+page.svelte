<script>
  ...
  const WEB3FORMS_ACCESS_KEY = '55529fac-686c-4861-828e-3245f3c92131'; 
  ...
  const socialLinks = [
    ...
    { label: 'Email', icon: 'mail', to: 'mailto:norbertbrett@outlook.com' }
  ];
...
<div bind:this={pageRef} onmousemove={handleMouseMove} class="min-h-screen text-white relative overflow-x-hidden pt-20" role="main">
  ...
  <!-- Ambient orbs -->
  <div class="absolute top-[5%] right-[5%] w-[50vw] h-[50vw] bg-primary/[0.025] rounded-full blur-[180px] pointer-events-none"></div>
  <div class="absolute bottom-[10%] left-[0%] w-[40vw] h-[40vw] bg-white/[0.01] rounded-full blur-[150px] pointer-events-none"></div>
  ...
  <section class="relative pt-32 sm:pt-44 pb-16 sm:pb-24 px-6 sm:px-12">
    <div class="max-w-7xl mx-auto relative z-10">
      <div class="ct-eyebrow flex items-center gap-4 mb-8">
        ...
        <span class="text-[10px] font-mono text-primary/70 tracking-[0.5em] uppercase">Contact</span>
        <div class="inline-flex items-center gap-2 px-3 py-1 border border-primary/20 bg-primary/[0.05] rounded-full ml-4">
          <span class="relative flex h-2 w-2">
            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span>
            <span class="relative inline-flex rounded-full h-2 w-2 bg-primary"></span>
          </span>
          <span class="text-[9px] font-mono text-primary/90 tracking-widest uppercase font-bold">Available</span>
        </div>
      </div>

      <h1 class="text-[14vw] md:text-[10vw] lg:text-[9rem] font-black font-serif tracking-tighter leading-[0.85] mb-10 overflow-hidden">
        <span class="block">
    ...
    <div class="max-w-7xl mx-auto mt-14 sm:mt-20">
      <div class="w-full h-px bg-gradient-to-r from-primary/20 via-white/5 to-transparent"></div>
    </div>
  </section>
      ...
      <div class="grid lg:grid-cols-12 gap-16 lg:gap-24">
        <div class="lg:col-span-5 space-y-12">
          <div class="ct-info-item">
            <span class="text-[9px] font-mono text-white/40 tracking-[0.5em] uppercase block mb-4 uppercase">Direct Link</span>
            <a href="mailto:norbertbrett@outlook.com" class="group flex items-center gap-3 interactive">
              <span class="text-xl md:text-2xl font-serif font-bold text-white group-hover:text-primary transition-colors duration-300">norbertbrett@outlook.com</span>
            ...
            <p class="text-sm text-white/50 font-mono mt-3 leading-relaxed">
              Based in Budapest, Hungary<br>Operations: Global
            </p>
          </div>

          <div class="ct-info-item">
            <span class="text-[9px] font-mono text-white/40 tracking-[0.5em] uppercase block mb-5 uppercase">Services</span>
            <div class="grid grid-cols-2 gap-3">
              {#each services as svc}
                <div class="group flex items-center gap-3 p-4 rounded-xl border border-white/5 bg-white/[0.015] hover:border-primary/20 hover:bg-white/[0.03] transition-all duration-400">
                  ...
                  <span class="text-xs font-mono text-white/70 uppercase tracking-wider leading-tight uppercase">{ svc.label }</span>
                </div>
              {/each}
            </div>
          </div>

          <div class="ct-info-item p-6 rounded-2xl border border-white/5 bg-white/[0.015]">
            <div class="flex items-center gap-4 mb-3">
              ...
              <div>
                <span class="text-sm font-serif font-bold text-white block">Response Time</span>
                <span class="text-[10px] font-mono text-white/50 uppercase tracking-wider uppercase">Within 24 Hours • Mon – Fri</span>
              </div>
            </div>
          </div>

          <div class="ct-info-item">
            <span class="text-[9px] font-mono text-white/40 tracking-[0.5em] uppercase block mb-5 uppercase">Channels</span>
            <div class="flex gap-3">
              {#each socialLinks as link}
                <a href={link.to} target="_blank" class="ct-social group flex items-center gap-3 px-5 py-3.5 rounded-xl border border-white/5 bg-white/[0.015] hover:border-primary/30 hover:bg-white/[0.04] transition-all duration-400 interactive">
                  ...
                  {/if}
                  <span class="text-xs font-mono uppercase tracking-wider text-white/70 group-hover:text-white transition-colors uppercase">{ link.label }</span>
                </a>
              {/each}
            </div>
          </div>
        ...
        <div class="lg:col-span-7">
          <div class="ct-form-card spotlight-card relative p-8 md:p-12 rounded-3xl border border-white/5 bg-white/[0.02] hover:border-primary/10 transition-all duration-500 overflow-hidden shadow-2xl">
            <div class="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl" style="background: radial-gradient(500px circle at var(--mx, 50%) var(--my, 50%), rgba(212,175,55,0.04), transparent 50%);"></div>

            <div class="relative z-10 flex items-center justify-between mb-10">
              <div class="flex items-center gap-3">
                <span class="w-2 h-2 bg-primary rounded-full animate-pulse"></span>
                <span class="text-[10px] font-mono uppercase tracking-[0.4em] text-white/50 uppercase">Send a Message</span>
              </div>
              <span class="text-[9px] font-mono text-white/30 tracking-wider uppercase hidden sm:block uppercase">Secure Form</span>
            </div>
            ...
            {#if isSuccess}
              <div class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/95 backdrop-blur-md rounded-3xl transition-all duration-500">
                ...
                <h3 class="text-3xl font-serif font-black mb-3 text-white uppercase">Message Sent</h3>
                <p class="text-white/60 font-mono text-sm uppercase mb-10 tracking-wider uppercase">I'll reply within 24 hours.</p>
                <Button variant="outline" class="px-8 py-3 rounded-full border border-white/20 text-sm font-mono uppercase tracking-widest hover:border-primary hover:text-primary transition-all duration-300" onclick={() => isSuccess = false}>
                  Send Another
                </Button>
              </div>
            {/if}
            ...
            <form onsubmit={onSubmit} class="space-y-8 relative z-10">
              <div class="grid md:grid-cols-2 gap-6">
                <div class="space-y-2">
                  <Label for="name" class="uppercase font-mono text-[10px] tracking-widest text-white/40 uppercase">Name</Label>
                  <Input id="name" bind:value={state.name} placeholder="Your name" class="bg-white/[0.03] border-white/10 text-white rounded-xl h-12" required />
                </div>
                <div class="space-y-2">
                  <Label for="email" class="uppercase font-mono text-[10px] tracking-widest text-white/40 uppercase">Email</Label>
                  <Input id="email" type="email" bind:value={state.email} placeholder="your@email.com" class="bg-white/[0.03] border-white/10 text-white rounded-xl h-12" required />
                </div>
              </div>

              <div class="space-y-2">
                <Label for="message" class="uppercase font-mono text-[10px] tracking-widest text-white/40 uppercase">Message</Label>
                <Textarea id="message" bind:value={state.message} placeholder="Tell me about your project..." class="bg-white/[0.03] border-white/10 text-white rounded-xl min-h-[150px]" required />
              </div>

              <div class="pt-2">
                <Button type="submit" size="lg" disabled={isSubmitting} class="w-full flex items-center justify-center gap-3 group relative overflow-hidden bg-primary text-black font-mono font-bold uppercase text-sm h-14 hover:bg-[#b8983f] transition-all duration-400 rounded-full tracking-widest shadow-[0_0_40px_rgba(212,175,55,0.15)] hover:shadow-[0_0_60px_rgba(212,175,55,0.25)]">
                  {#if !isSubmitting}
                ...
                <p class="text-center mt-4 text-[9px] font-mono text-white/30 tracking-wider uppercase">
